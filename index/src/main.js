...
/**
 * Maze and Bird have extra levels beyond what the other apps share via
 * BlocklyGames.MAX_LEVEL.
...
function init() {
  ...
  for (let i = 0; i < APPS.length; i++) {
    levelsDone[i] = 0;
    for (let j = 1; j <= maxLevelFor(APPS[i]); j++) {
  ...
  for (let i = 0; i < levelsDone.length; i++) {
    const app = APPS[i];
    const denominator = (i === 0) ? 1 : maxLevelFor(app);
